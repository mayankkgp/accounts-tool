...
// EntityCreateDrawer triggers quick and compact single-panel entity routing records for new registers
export default function EntityCreateDrawer({ isOpen, onClose, onRefresh }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Parse & append secondary addresses sequentially on the database reference
      for (const r of extraAddresses) {
        if (r.trim()) {
          const p = r.split(",").map((x) => x.trim()).filter(Boolean);
          if (p.length) {
            const pin = p.find((x) => /^\d{5,6}$/.test(x)) || "";
            const parsed = { addressLine1: p[0] || "Main Office", city: p[1] || "Unknown", state: p[2] || "Unknown", pincode: pin || "000000" };
            await manageAddress(ent.id, { ...parsed, isDefaultBilling: false, isDefaultShipping: false });
          }
        }
      }
      onRefresh();
    ...
    <>
      ...
      <div className="fixed top-0 right-0 bottom-0 w-[420px] sm:w-[520px] bg-white border-l border-slate-200 shadow-2xl z-50 flex flex-col h-full font-sans text-xs" id="create-drawer-body">
        <div className="flex items-center justify-between p-3 border-b border-slate-200 shrink-0">
          ...
          <button type="button" onClick={onClose} className="h-5 w-5 rounded-sm flex items-center justify-center text-slate-400 hover:text-slate-600 hover:bg-slate-50" id="btn-close-create-drawer">
            <X size={12} />
          </button>
        </div>
              ...
              <div className="flex bg-slate-200/60 p-0.5 rounded-sm w-full">
                {["brand", "factory", "vendor"].map((t) => (
                  <button
                    ...
                    className={`flex-1 text-[9px] uppercase tracking-wider font-semibold py-1 rounded-[1px] transition-all cursor-pointer outline-none text-center ${
                      formData.type === t ? "bg-slate-950 text-slate-100 shadow-sm font-bold" : "text-slate-600 hover:text-slate-900 hover:bg-slate-300/40"
                    }`}
                  >
                    {t}
                  </button>
