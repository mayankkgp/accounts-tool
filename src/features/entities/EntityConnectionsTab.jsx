...
export default function EntityConnectionsTab({ profile, onRefresh }) {
  ...
  if (profile.type === "vendor") {
    return (
      <div className="p-4 border border-slate-100 bg-slate-50 rounded-sm text-center text-slate-550 font-medium text-[10px] italic select-none">
  ...
  return (
    <div className="space-y-3 flex flex-col flex-1 font-sans" id="connections-tab-container" ref={containerRef}>
      <div className="flex flex-col gap-1.5">
        <label className="text-[10px] uppercase font-bold text-slate-500 select-none">Link Mapped {targetLabel}</label>
        <div className="relative w-full">
          <div className="relative flex items-center h-6 border border-slate-205 rounded-sm bg-white focus-within:border-indigo-500 transition-all px-1.5">
            ...
            />
            {searchQuery && (
              <button type="button" onClick={() => setSearchQuery("")} className="text-slate-400 hover:text-slate-650 outline-none">
                <X size={10} />
              </button>
            )}
          </div>
          {isOpen && (
            <div className="absolute top-7 left-0 right-0 z-50 bg-white border border-slate-200 shadow-md rounded-sm max-h-[120px] overflow-y-auto mt-0.5">
              {filteredCandidates.length === 0 ? (
                <div className="px-2 py-1.5 text-[10px] text-slate-400 italic font-medium">No options matching query</div>
              ) : (
                filteredCandidates.map((item) => (
                  ...
                  >
                    <span className="truncate text-slate-805 font-medium">{item.brandName ? `${item.brandName} (${item.businessName})` : item.businessName}</span>
      ...
      <div className="flex flex-col gap-1.5">
        <span className="text-[10px] uppercase font-bold text-slate-555 select-none">Active Relations ({localConnections.length})</span>
        {localConnections.length === 0 ? (
          ...
          <div className="flex flex-wrap gap-1" id="active-tags-grid">
            {localConnections.map((conn) => (
              ...
              >
                <span className="truncate max-w-[125px]" title={conn.businessName}>{conn.brandName ? `${conn.brandName} (${conn.businessName})` : conn.businessName}</span>
                <button
                  type="button"
